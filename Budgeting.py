...
for fileName in glob.glob('C:\\users\\sariz\\onedrive\\desktop\\myPythonScripts\\budgetCSV\**\*.csv', recursive= True):
    ...
    os.remove('C:\\users\\sariz\\onedrive\\desktop\\myPythonScripts\\budgetCSV\\Transactions.csv')    
    ...
    for itemNum in range(len(itemLists)):
        for itemScroll in range(len(date)):
            ws1.cell(row= rowNum, column= columnNum).value = itemLists[itemNum][itemScroll]
            rowNum += 1
        columnNum += 1
        rowNum = 3
